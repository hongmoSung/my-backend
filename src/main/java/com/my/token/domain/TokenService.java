...
public class TokenService {
	...
	private String getTokenString(Token token) throws JsonProcessingException {
		...
		return Jwts.builder().signWith(getSecretKey()).content(content).compact();
	}
	...
	public Token decodeToken(String tokenString) {
		Claims claims = Jwts.parser().verifyWith(getSecretKey()).build().parseSignedClaims(tokenString).getPayload();

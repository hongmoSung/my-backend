...
class PayControllerTest extends RestDocTestSupport {
	...
	void getBalance() throws Exception {
		...
		ResPayDto res = ResPayDto.builder().email("a@a.com").money(BigInteger.ZERO).build();
		given(payFacade.getBalance(authorization)).willReturn(res);

		mockMvc
			.perform(get("/api/v1/pays/balance").contentType(MediaType.APPLICATION_JSON)
				.header("Authorization", authorization))
			.andDo(print())
			.andExpect(status().isOk())
			.andDo(document("get-balance",
					responseFields(fieldWithPath("email").type(JsonFieldType.STRING).description("유저 이메일"),
	...
	void reCharge() throws Exception {
		...
		mockMvc
			.perform(put("/api/v1/pays/balance-recharge").contentType(MediaType.APPLICATION_JSON)
				.header("Authorization", authorization)
				.content(objectMapper.writeValueAsString(requestBody)))
			.andDo(print())
			.andExpect(status().isNoContent())
			.andDo(document("balance-recharge",
					requestFields(fieldWithPath("chargeAmount").type(JsonFieldType.NUMBER).description("충전할 금액"))));
	}
	...
	void pay() throws Exception {
		...
		mockMvc
			.perform(put("/api/v1/pays/{bookingId}", 1L).contentType(MediaType.APPLICATION_JSON)
				.header("Authorization", authorization)
				.content(objectMapper.writeValueAsString(requestBody)))
			.andDo(print())
			.andExpect(status().isNoContent())
			.andDo(document("payment", pathParameters(parameterWithName("bookingId").description("예약 id"))));
	}
